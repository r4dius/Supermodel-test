...
in vec4 subTexture;      // .x=texture X, .y=texture Y, .z=texture width, .w=texture height (all in texels)
in vec4 texParams;       // .x=texture enable (if 1, else 0), .y=use transparency (if >=0), .z=U wrap mode (1=mirror, 0=repeat), .w=V wrap mode
in float texFormat;      // T1RGB5 contour texture (if > 0)
in float texMap;         // texture map number
in float transLevel;     // translucence level, 0.0 (transparent) to 1.0 (opaque). if less than 1.0, replace alpha value
in float lightEnable;    // lighting enabled (1.0) or luminous (0.0), drawn at full intensity
in float specular;       // specular coefficient (0.0 if disabled)
in float shininess;      // specular shininess
in float fogIntensity;   // fog intensity (1.0, full fog effect, 0.0, no fog) 

// Custom outputs to fragment shader
out vec4 fsSubTexture;
out vec4 fsTexParams;
out float fsTexFormat;
out float fsTexMap;
out float fsTransLevel;
out vec3 fsLightIntensity;   // total light intensity for this vertex
out float fsSpecularTerm;    // specular light term (additive)
out float fsFogFactor;       // fog factor
out float fsViewZ;
...
void main(void) {
    ...
    viewVertex = vec3(modelViewMatrix * vec4(gl_Vertex, 1.0));  
